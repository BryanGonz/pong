...
simulate_player(float *p, float* dp, float ddp, float dt) {
	...
	*dp = *dp + ddp * dt;//calculates movement
...
} // sets up the two paddles

internal bool
aabb_vs_aabb(float p1x, float p1y, float hs1x, float hs1y,
	float p2x, float p2y, float hs2x, float hs2y) {
	return(p1x + hs1x > p2x - hs2x &&
		   p1x - hs1x < p2x + player_half_size_x &&
		   p1y + hs1y > p2y - hs2y &&
		   p1y + hs1y < p2y + hs2y);
}
...
simulate_game(Input* input, float dt) {
	...
	// the ball changes direction based on where it landed on the paddle then it looks at how fast the paddle was moving
	...
	//this will check if the ball hit the top or bottom of the arena
	...
	//this resets the ball after a score
	...
	draw_number(player_1_score, -10, 40, 1.f, 0xbbff);
	draw_number(player_2_score, 10, 40, 1.f, 0xbbff);
